...
class BiLSTM:
    ...
    def setDataset(self, datasets, data):
        ...
        self.epoch = 0
        self.learning_rate_updates = {'sgd': {1: 0.5, 3: 0.05, 5: 0.01}}
...
#                         output = LSTM(size, return_sequences=True)(output)#, name=modelName+'_LSTM_'+str(cnt))(output) 
    ...
    def do_stuff(self, start_seq):
        start_seq = [{'tokens': nltk.word_tokenize(sent)} for sent in nltk.sent_tokenize(start_seq)]
        addCharInformation(start_seq)
        addCasingInformation(start_seq)
        dataMatrix = createMatrices(start_seq, lstmModel.mappings, True)
        prediction = 1111
        nn_input = []
        while prediction != 9:
            nn_input.append([dataMatrix[0]['tokens']])
            nn_input.append([dataMatrix[0]['casing']])
            nn_input_array = np.asarray(nn_input)
            prediction = model.predict(nn_input_array, verbose=False)
            predictions = predictions.argmax(axis=-1) #für eine Sequenz, die predicteten wort-ids
        



    def tagSentences(self, sentences): #aufgerufen in run mit sentences = datamatrices
        # Pad characters
        if 'characters' in self.params['featureNames']:
            self.padCharacters(sentences)
        print('input von tagSentences: ', sentences)
        labels = {}
        for modelName, model in self.models.items():
            paddedPredLabels = self.generate_text(model, sentences)
            predLabels = []
#             for idx in range(len(sentences)):
#                 unpaddedPredLabels = []
...
#                 predLabels.append(unpaddedPredLabels)
    ...
    def generate_text(self, model, sentences): #sentences = Sätze aus Textdatei
        predLabels = [None]*len(sentences)
        sentenceLengths = self.getSentenceLengths(sentences) # Dictionary mit länge_sätze:index der sätze mit länge 
        
        for indices in sentenceLengths.values():   
            nnInput = []                  
            for featureName in self.params['featureNames']:
                print('featureName: ', featureName)
                inputData = np.asarray([sentences[idx][featureName] for idx in indices]) # input data = wort-ids des Inputs
                nnInput.append(inputData)
            print('Final Input to NN: ', nnInput)
            
            predictions = model.predict(nnInput, verbose=False)
            print('typ und laenge Predictions: ', type(predictions), np.shape(predictions))
            ...
            for idx in indices:
                ...
                predIdx += 1   
        print('return value predLabels: ',predLabels)
        return predLabels

        
    def predictLabels(self, model, sentences): #sentences = Sätze aus Textdatei
        predLabels = [None]*len(sentences)
        sentenceLengths = self.getSentenceLengths(sentences) # Dictionary mit länge_sätze:index der sätze mit länge 
        ...
        for indices in sentenceLengths.values():   
            ...
            for idx in indices:
                predLabels[idx] = predictions[predIdx]    
                predIdx += 1   
        return predLabels # liste der Predicteten IDs
